...
public class PokerHand {
    ...
    public String toString() {
        return "%d. %-16s Rank:%d %-40s Best:%-7s Worst:%-6s %s".formatted(playerNo,score,score.ordinal(),hand
                ,Collections.max(hand, Comparator.comparing(Card::getRank)),Collections.min(hand, Comparator.comparing(Card::getRank))
    ...
    private void pickDisards(){
        ...
        int index=0;
        for (Card c:temp) {
            if (index++<3&&(rankedCards>2||c.getRank()<9))discards.add(c);
            else keepers.add(c);
        }
    }
}
